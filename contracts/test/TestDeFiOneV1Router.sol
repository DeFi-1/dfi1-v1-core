...
contract TestDeFiOneV1Router is IDeFiOneV1SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IDeFiOneV1Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function defioneV1SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IDeFiOneV1Pool(msg.sender).token0() : IDeFiOneV1Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IDeFiOneV1Pool(pools[0]).token1();
            IDeFiOneV1Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IDeFiOneV1Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IDeFiOneV1Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
